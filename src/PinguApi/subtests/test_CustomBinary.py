...
class CustomBinaryTests(APITestCase):
    def setUp(self):
    ...
    @patch('PinguApi.tasks.upload_custom_binary_to_bucket.apply')
    def test_add_custom_binary_to_job(self, mock_apply: MagicMock):
        custom_binary_payload = {
            'job_id': self.test_job.id,
            'custom_binary': base64.b64encode(self.custom_binary_zip).decode('utf-8'),
            'file_name': 'test.zip'
        }
        ...
        response = self.client.post(f'/api/custom_binary/', data=custom_binary_payload, format='json')
    ...
    @patch('PinguApi.tasks.remove_custom_binary_from_bucket.apply')
